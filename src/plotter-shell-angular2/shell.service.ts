...
import { PlotterShellModel, StateDirectory, ViewInstance, ViewInstanceJSON } from '../plotter-shell-model';
...
export class ShellService {
    ...
    public headerViewInstances = new Array<ViewInstance>();
    public footerViewInstances = new Array<ViewInstance>();
        ...
        if (!reuseTab) {
            ...
            if (foundViewInstance) { this.replaceViewInstance(foundViewInstance, viewInstance); }
                ...
                if (foundViewInstance) { this.replaceViewInstance(foundViewInstance, viewInstance); }
                    ...
                    foundViewInstance = this.navViewInstances.find(vi => vi.uniqueId === viewInstance.uniqueId);
                    if (foundViewInstance) { this.replaceViewInstance(foundViewInstance, viewInstance); }
                    else {
                        foundViewInstance = this.headerViewInstances.find(vi => vi.uniqueId === viewInstance.uniqueId);
                        if (foundViewInstance) { this.replaceViewInstance(foundViewInstance, viewInstance); }
                        else {
                            foundViewInstance = this.footerViewInstances.find(vi => vi.uniqueId === viewInstance.uniqueId);
                            if (foundViewInstance) { this.replaceViewInstance(foundViewInstance, viewInstance); }
    ...
    public launchViewInstanceInNewTab = (viewInstance: ViewInstance) => {
        switch (viewInstance.paneType) {
            ...
            case 'header':
                this.headerViewInstances.push(viewInstance);
                break;
